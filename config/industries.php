<?php

return [
    'default' => 'general',

    'list' => [
        'general' => 'General / Other',
        'estate-agents' => 'Estate agents',
        'property-management' => 'Property management',
        'construction' => 'Construction',
        'electricians' => 'Electricians',
        'plumbers' => 'Plumbers',
        'cleaning-companies' => 'Cleaning companies',
        'roofers' => 'Roofers',
        'landscapers' => 'Landscapers',
        'facilities-management' => 'Facilities management',
        'care-agencies' => 'Care agencies',
        'restaurants' => 'Restaurants',
        'hotels' => 'Hotels',
        'retail' => 'Retail',
        'marketing-agencies' => 'Marketing agencies',
        'software-development' => 'Software development',
        'accountants' => 'Accountants',
        'recruitment' => 'Recruitment',
        'schools' => 'Schools',
        'charities' => 'Charities',
        'manufacturing' => 'Manufacturing',
    ],

    /*
    | Master todo type definitions (icon = Lucide icon name for the frontend).
    */
    'types' => [
        'Bug' => ['icon' => 'Bug'],
        'Feature' => ['icon' => 'Zap'],
        'Task' => ['icon' => 'CheckSquare'],
        'Story' => ['icon' => 'BookOpen'],
        'Epic' => ['icon' => 'Layers'],
        'Property' => ['icon' => 'Home'],
        'Lead' => ['icon' => 'UserPlus'],
        'Viewing' => ['icon' => 'Eye'],
        'Offer' => ['icon' => 'HandCoins'],
        'Maintenance' => ['icon' => 'Wrench'],
        'Compliance' => ['icon' => 'ShieldCheck'],
        'Tenant' => ['icon' => 'Users'],
        'Inspection' => ['icon' => 'ClipboardCheck'],
        'Lease' => ['icon' => 'FileText'],
        'Site Visit' => ['icon' => 'MapPin'],
        'Snagging' => ['icon' => 'Hammer'],
        'Safety' => ['icon' => 'AlertTriangle'],
        'Material' => ['icon' => 'Package'],
        'Subcontractor' => ['icon' => 'HardHat'],
        'Job' => ['icon' => 'Briefcase'],
        'Quote' => ['icon' => 'FileText'],
        'Install' => ['icon' => 'Wrench'],
        'Repair' => ['icon' => 'Hammer'],
        'Emergency' => ['icon' => 'AlertTriangle'],
        'Deep Clean' => ['icon' => 'Sparkles'],
        'Regular Clean' => ['icon' => 'Sparkles'],
        'Supply' => ['icon' => 'Package'],
        'Survey' => ['icon' => 'ClipboardList'],
        'Warranty' => ['icon' => 'ShieldCheck'],
        'Design' => ['icon' => 'PenTool'],
        'Seasonal' => ['icon' => 'TreePine'],
        'Work Order' => ['icon' => 'ClipboardList'],
        'Vendor' => ['icon' => 'Truck'],
        'Client' => ['icon' => 'User'],
        'Visit' => ['icon' => 'MapPin'],
        'Assessment' => ['icon' => 'ClipboardCheck'],
        'Medication' => ['icon' => 'Pill'],
        'Training' => ['icon' => 'GraduationCap'],
        'Prep' => ['icon' => 'ChefHat'],
        'Service' => ['icon' => 'ConciergeBell'],
        'Stock' => ['icon' => 'Package'],
        'Guest Request' => ['icon' => 'ConciergeBell'],
        'Housekeeping' => ['icon' => 'Sparkles'],
        'Event' => ['icon' => 'Calendar'],
        'Display' => ['icon' => 'LayoutGrid'],
        'Customer' => ['icon' => 'User'],
        'Delivery' => ['icon' => 'Truck'],
        'Campaign' => ['icon' => 'Megaphone'],
        'Creative' => ['icon' => 'Palette'],
        'Content' => ['icon' => 'FileText'],
        'Analytics' => ['icon' => 'BarChart3'],
        'Meeting' => ['icon' => 'Users'],
        'Tax' => ['icon' => 'Calculator'],
        'Audit' => ['icon' => 'Search'],
        'Bookkeeping' => ['icon' => 'BookOpen'],
        'Candidate' => ['icon' => 'UserPlus'],
        'Interview' => ['icon' => 'Calendar'],
        'Placement' => ['icon' => 'CheckCircle'],
        'Admin' => ['icon' => 'ClipboardList'],
        'Lesson' => ['icon' => 'BookOpen'],
        'Student' => ['icon' => 'GraduationCap'],
        'Parent' => ['icon' => 'Users'],
        'Fundraising' => ['icon' => 'Heart'],
        'Volunteer' => ['icon' => 'HeartHandshake'],
        'Production' => ['icon' => 'Factory'],
        'Quality' => ['icon' => 'ShieldCheck'],
        'Order' => ['icon' => 'ShoppingCart'],
        'Inventory' => ['icon' => 'Package'],
        'Project' => ['icon' => 'FolderKanban'],
        'Follow-up' => ['icon' => 'Phone'],
        'Other' => ['icon' => 'CircleHelp'],
    ],

    'industry_types' => [
        'general' => ['Task', 'Project', 'Meeting', 'Follow-up', 'Compliance', 'Other'],
        'estate-agents' => ['Property', 'Lead', 'Viewing', 'Offer', 'Maintenance', 'Compliance', 'Other'],
        'property-management' => ['Property', 'Tenant', 'Maintenance', 'Inspection', 'Compliance', 'Lease', 'Other'],
        'construction' => ['Site Visit', 'Task', 'Snagging', 'Safety', 'Material', 'Subcontractor', 'Compliance', 'Other'],
        'electricians' => ['Job', 'Quote', 'Install', 'Repair', 'Inspection', 'Emergency', 'Compliance', 'Other'],
        'plumbers' => ['Job', 'Quote', 'Install', 'Repair', 'Inspection', 'Emergency', 'Compliance', 'Other'],
        'cleaning-companies' => ['Job', 'Quote', 'Deep Clean', 'Regular Clean', 'Inspection', 'Supply', 'Compliance', 'Other'],
        'roofers' => ['Job', 'Quote', 'Survey', 'Install', 'Repair', 'Warranty', 'Compliance', 'Other'],
        'landscapers' => ['Job', 'Quote', 'Design', 'Install', 'Maintenance', 'Seasonal', 'Compliance', 'Other'],
        'facilities-management' => ['Work Order', 'Inspection', 'Maintenance', 'Compliance', 'Vendor', 'Emergency', 'Other'],
        'care-agencies' => ['Client', 'Visit', 'Assessment', 'Medication', 'Compliance', 'Training', 'Other'],
        'restaurants' => ['Task', 'Prep', 'Service', 'Stock', 'Maintenance', 'Compliance', 'Other'],
        'hotels' => ['Task', 'Guest Request', 'Housekeeping', 'Maintenance', 'Event', 'Compliance', 'Other'],
        'retail' => ['Task', 'Stock', 'Display', 'Customer', 'Delivery', 'Maintenance', 'Compliance', 'Other'],
        'marketing-agencies' => ['Campaign', 'Creative', 'Client', 'Content', 'Analytics', 'Meeting', 'Compliance', 'Other'],
        'software-development' => ['Bug', 'Feature', 'Task', 'Story', 'Epic', 'Compliance', 'Other'],
        'accountants' => ['Client', 'Tax', 'Audit', 'Bookkeeping', 'Compliance', 'Meeting', 'Other'],
        'recruitment' => ['Candidate', 'Client', 'Interview', 'Placement', 'Compliance', 'Admin', 'Other'],
        'schools' => ['Lesson', 'Admin', 'Student', 'Parent', 'Event', 'Compliance', 'Other'],
        'charities' => ['Campaign', 'Fundraising', 'Volunteer', 'Event', 'Admin', 'Compliance', 'Other'],
        'manufacturing' => ['Production', 'Quality', 'Maintenance', 'Order', 'Safety', 'Inventory', 'Compliance', 'Other'],
    ],

    /*
    | Unsplash search queries for subdomain homepage hero backgrounds (landscape).
    */
    'background_queries' => [
        'general' => 'professional office team workspace',
        'estate-agents' => 'modern house exterior residential property',
        'property-management' => 'apartment building property management',
        'construction' => 'construction site building workers',
        'electricians' => 'electrician wiring installation',
        'plumbers' => 'plumber plumbing repair',
        'cleaning-companies' => 'professional cleaning service office',
        'roofers' => 'roofing construction house',
        'landscapers' => 'landscaping garden design',
        'facilities-management' => 'commercial building facilities',
        'care-agencies' => 'healthcare care home support',
        'restaurants' => 'restaurant kitchen dining',
        'hotels' => 'hotel lobby hospitality',
        'retail' => 'retail store shopping',
        'marketing-agencies' => 'creative agency marketing team',
        'software-development' => 'software developer coding office',
        'accountants' => 'accounting finance office',
        'recruitment' => 'recruitment interview office',
        'schools' => 'school classroom education',
        'charities' => 'charity community volunteering',
        'manufacturing' => 'manufacturing factory production',
    ],

    /*
    | Static fallback hero images when Unsplash API is unavailable.
    */
    'background_fallbacks' => [
        'general' => 'https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&w=1600&q=80',
        'estate-agents' => 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&w=1600&q=80',
        'property-management' => 'https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?auto=format&fit=crop&w=1600&q=80',
        'construction' => 'https://images.unsplash.com/photo-1504307651254-35680f356dfd?auto=format&fit=crop&w=1600&q=80',
        'electricians' => 'https://images.unsplash.com/photo-1621905251189-08b45d6a269e?auto=format&fit=crop&w=1600&q=80',
        'plumbers' => 'https://images.unsplash.com/photo-1585704032915-c3400ca2765d?auto=format&fit=crop&w=1600&q=80',
        'cleaning-companies' => 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?auto=format&fit=crop&w=1600&q=80',
        'roofers' => 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=1600&q=80',
        'landscapers' => 'https://images.unsplash.com/photo-1558904541-efa843a96f01?auto=format&fit=crop&w=1600&q=80',
        'facilities-management' => 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&w=1600&q=80',
        'care-agencies' => 'https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?auto=format&fit=crop&w=1600&q=80',
        'restaurants' => 'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?auto=format&fit=crop&w=1600&q=80',
        'hotels' => 'https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=1600&q=80',
        'retail' => 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?auto=format&fit=crop&w=1600&q=80',
        'marketing-agencies' => 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&w=1600&q=80',
        'software-development' => 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&w=1600&q=80',
        'accountants' => 'https://images.unsplash.com/photo-1450101499163-c8848c66ca85?auto=format&fit=crop&w=1600&q=80',
        'recruitment' => 'https://images.unsplash.com/photo-1521737711867-e3b97375f902?auto=format&fit=crop&w=1600&q=80',
        'schools' => 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=1600&q=80',
        'charities' => 'https://images.unsplash.com/photo-1469571480202-5b953214599d?auto=format&fit=crop&w=1600&q=80',
        'manufacturing' => 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&w=1600&q=80',
    ],
];
